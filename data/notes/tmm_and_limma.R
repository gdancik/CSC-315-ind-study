################################################################
# tmm_and_limma.R. This is a LONG script that goes through 
# the process of 
#
# 1) reading UCSC Xena data into R
# 2) pre-processing the data (matching expression and phenotypes)
# 3) processing the count data using TMM
# 4) finding differentially expressed genes using limma 
# 
# For Limma, we will generally follow the process described in 
# Chapter 15 of the limma user manual, using limma trend. 
# To access this, run the command: limmaUsersGuide()
################################################################
...
##############################################################################
# 1) Retrieve genomic and phenotype data from UCSC Xena
##############################################################################
...
#######################################################
# (3) Process the expression data using TMM
#######################################################
...
# about 3-fold. This is the approach that we will use, even though
# the ratio for us is around 3.9

max(dge$samples$norm.factors) / min(dge$samples$norm.factors)

# An alternative method called limma voom may be a little 
# better for ratio differences above 3-fold; but the results in this 
...
# Note that all genes are on the X or Y chromosomes; 
...
# set the row rownames to gene name (chromosome);
# normally chromosomes should not be included, but here
# they are relevant for denoting sex differences
...
# we use %in% rather than match because there may be multiple
# matches
...
# This gene is up-regulated in females OR This gene is down-regulated in males
